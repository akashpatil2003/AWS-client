...
const LeaveDetail = () => {
  ...
  useEffect(() => {
    const fetchleave = async () => {
      try {
        const response = await axios.get(`https://aws-server-dusky.vercel.app/api/leave/detail/${id}`, {
          headers: {
  ...
  const changeStatus = async (id, status) => {
    try {
      const response = await axios.put(`https://aws-server-dusky.vercel.app/api/leave/${id}`, { status }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
        }
      });
